...
def get_settings(db: Session = Depends(get_db)) -> SettingsOut:
    """Return non-secret runtime settings (masked key + AI status)."""
    key = settings_service.get_gemini_key(db)
    return SettingsOut(
        gemini_key_set=bool(key),
        gemini_key_masked=settings_service.mask_secret(key),
        ai_enabled=ai_service.ai_enabled(db),
    )
...
def update_settings(
    ...
    """Update runtime settings. The Gemini key is stored in the database and
    takes effect immediately (empty string clears it; omitted keeps it)."""
    if payload.gemini_api_key is not None:
        settings_service.set_setting(
            db, settings_service.GEMINI_KEY, payload.gemini_api_key.strip()
        )
    key = settings_service.get_gemini_key(db)
    return SettingsOut(
        gemini_key_set=bool(key),
...
def get_overview(db: Session = Depends(get_db)) -> AdminOverviewOut:
    ...
    revenue_sum = db.scalar(select(func.sum(Order.amount)).where(Order.status == OrderStatus.paid))
...
def update_user(user_id: int, payload: AdminUserUpdate, db: Session = Depends(get_db)) -> User:
    user = db.get(User, user_id)
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
...
@router.delete(
    "/levels/{level_id}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response
)
...
def update_question(
    ...
    if payload.options is not None:
        ...
        for opt_in in payload.options:
            option = Option(
                label=opt_in.label, content=opt_in.content, is_correct=opt_in.is_correct
            )
            question.options.append(option)
...
@router.delete(
    "/questions/{question_id}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response
)
...
def import_questions(payload: QuestionImportIn, db: Session = Depends(get_db)) -> QuestionImportOut:
    ...
    for q_row in payload.questions:
        ...
        for opt_row in q_row.options:
            option = Option(
                label=opt_row.label, content=opt_row.content, is_correct=opt_row.is_correct
            )
            question.options.append(option)
        db.add(question)
        count += 1
...
@router.delete(
    "/mock-tests/{mock_id}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response
)
...
def update_article(
    ...
    for field, value in payload.model_dump(exclude_unset=True).items():
        if field == "slug" and value != art.slug:
            ...
            if existing:
                raise HTTPException(
                    status_code=400, detail=f"Article slug '{value}' already exists"
                )
...
@router.delete(
    "/articles/{article_id}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response
)
...
def update_plan(plan_id: int, payload: PlanUpdate, db: Session = Depends(get_db)) -> Plan:
