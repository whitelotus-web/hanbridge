...
from app.models.content import Level, Skill, Section, Question
from app.models.billing import Order, Plan
from app.models.mock import MockTest
from app.models.article import Article
from app.models.enums import SkillType, QuestionType, OrderStatus


def _register_and_get_token(client: TestClient, email: str, is_staff: bool = False, db_session: Session | None = None) -> str:
    payload = {"email": email, "password": "password123", "display_name": "Admin User" if is_staff else "Regular User"}
...
def test_admin_overview(client: TestClient, db_session: Session) -> None:
    admin_token = _register_and_get_token(client, "admin@example.com", is_staff=True, db_session=db_session)
...
def test_admin_user_and_order_management(client: TestClient, db_session: Session) -> None:
    admin_token = _register_and_get_token(client, "admin2@example.com", is_staff=True, db_session=db_session)
    ...
    plan = Plan(name="Monthly Premium", interval="month", duration_days=30, price=9.99, currency="USD", is_active=True)
    db_session.add(plan)
    db_session.commit()
    db_session.refresh(plan)
    
    order = Order(user_id=user_id, plan_id=plan.id, status=OrderStatus.pending, amount=9.99, currency="USD", gateway="paypal")
    ...
    # Refund order
    res = client.post(f"/api/v1/admin/orders/{order.id}/refund", headers={"Authorization": f"Bearer {admin_token}"})
    assert res.status_code == 200
    assert res.json()["status"] == "refunded"


def test_admin_content_crud_and_import(client: TestClient, db_session: Session) -> None:
    admin_token = _register_and_get_token(client, "admin3@example.com", is_staff=True, db_session=db_session)
    ...
    res = client.post(
        ...
        json={"skill_id": skill_id, "title": "Advanced Match", "question_type": "match_picture", "order": 1, "is_free": False},
        headers={"Authorization": f"Bearer {admin_token}"}
    )
    assert res.status_code == 201
...
def test_admin_mock_tests_and_articles_crud(client: TestClient, db_session: Session) -> None:
    admin_token = _register_and_get_token(client, "admin4@example.com", is_staff=True, db_session=db_session)
    ...
    # Delete MockTest
    res = client.delete(f"/api/v1/admin/mock-tests/{mock_id}", headers={"Authorization": f"Bearer {admin_token}"})
